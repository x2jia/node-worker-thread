...
#define TYPEERROR isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, "param error", v8::NewStringType::kNormal).ToLocalChecked())); \
  return; \

#define TYPEERROR2(E) isolate->ThrowException(v8::Exception::TypeError(v8::String::NewFromUtf8(isolate, ("Error: "#E), v8::NewStringType::kNormal).ToLocalChecked())); \
  return; \
...
#define   THREAD     rcib::furOfThread* fur = node::ObjectWrap::Unwrap<rcib::furOfThread>(args.Holder()); \
                     base::Thread* thr = fur->thread(); \
                     if (!thr) { \
